...
describe("InfoService", () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [AngularFireModule.initializeApp(environment.firebase, APP_NAME)]
    });

    inject(
      [FirebaseApp, PlatformRef],
      (_app: FirebaseApp, _platform: PlatformRef) => {
        app = _app;
  ...
  describe("InfoService", () => {
    ...
    it("should use an already intialized firebase app if it exists", done => {
      @NgModule({
        imports: [
          AngularFireModule.initializeApp(environment.firebase, APP_NAME),
          BrowserModule
        ]
      })
      class MyModule {
